"""
DOC GENERATOR - PNA
doc_generator_pna.py

Word document version of the Product Not Acceptable chargeback response.
Same function signature as chargeback_generator_pna.generate_pdf.
"""
...
def _add_image_or_placeholder(doc, image_path, caption=None, max_width=6.0, max_height=3.5):
    if image_path and os.path.exists(image_path):
        try:
            ...
            doc.add_picture(image_path, width=Inches(display_width))
...
def generate_doc(data, output_path, tenant_name=None, screenshots=None):
    """
    Generate PNA chargeback Word document.
    ...
    sub = doc.add_paragraph("Product Unacceptable / Quality Dispute")
    ...
    if data.get('opening_statement'):
        ...
        doc.add_paragraph(
            ...
            "about the quality of the item or asked for a return, so the chargeback is invalid."
        )
        doc.add_paragraph()
        for item in ["Order details", "Payment proof", "Shipping proof",
    ...
    _add_image_or_placeholder(doc, screenshots.get('order_screenshot'), 'Shopify Order Details', max_height=6.0)
    doc.add_paragraph()
